...
FROM (VALUES
    ('tt15239678',9), ('tt15398776',10), ('tt14230458',9), ('tt28607951',8),
    ('tt7160372',10),  ('tt13238346',9),  ('tt0079944',10), ('tt0118767',9),
    ('tt2802154',9),   ('tt0047478',10),  ('tt0068646',10), ('tt0111161',10),
    ('tt0110912',10),  ('tt0816692',9),   ('tt6751668',10), ('tt2788316',9),
    ('tt7366338',10),  ('tt0033467',9),   ('tt0944947',8),  ('tt0903747',10),
    ('tt3032476',9),   ('tt0141842',10),  ('tt28015403',8)
) AS v(imdb,val)
...
FROM (VALUES
    ('tt15239678',8), ('tt15398776',9), ('tt14230458',7), ('tt28607951',9),
    ('tt7160372',8),  ('tt13238346',8), ('tt1517268',6), ('tt17279496',7),
    ('tt0079944',9),  ('tt0118767',8),  ('tt0231507',7), ('tt2802154',8),
    ('tt0068646',10), ('tt0111161',10), ('tt0110912',9), ('tt0468569',9),
    ('tt0167260',9),  ('tt0816692',8),  ('tt6751668',9), ('tt2788316',9),
    ('tt0903747',10), ('tt7366338',10), ('tt1475582',9)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
FROM (VALUES
    ('tt2788316',10), ('tt28015403',9), ('tt8772296',8),  ('tt4574334',9),
    ('tt0944947',10), ('tt0903747',10), ('tt3032476',9),  ('tt7366338',10),
    ('tt4786824',8),  ('tt10919420',9), ('tt0141842',9),  ('tt0773262',8),
    ('tt0108778',9),  ('tt1475582',10), ('tt0475784',8),
    -- немного фильмов
    ('tt0468569',9), ('tt0816692',8), ('tt15239678',7)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
FROM (VALUES
    ('tt15239678',9), ('tt1517268',8), ('tt17279496',7), ('tt0468569',10),
    ('tt0167260',10), ('tt0816692',10),('tt0120689',9), ('tt0118767',8),
    ('tt0231507',9),  ('tt0066565',9), ('tt0944947',8), ('tt0903747',8),
    ('tt4574334',8),  ('tt10919420',7),('tt0108778',9), ('tt2788316',8)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
FROM (VALUES
    ('tt15398776',10), ('tt14230458',10), ('tt7160372',10), ('tt13238346',10),
    ('tt0079944',10),  ('tt2802154',10),  ('tt0047478',10), ('tt0036824',9),
    ('tt14549466',9),  ('tt0033467',10),  ('tt6751668',10), ('tt28607951',9),
    -- мейнстрим — низко
    ('tt1517268',5),   ('tt17279496',6),  ('tt0468569',7),  ('tt15239678',6),
    ('tt0108778',5),   ('tt10919420',6)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
FROM (VALUES
    ('tt2788316',10), ('tt28015403',10), ('tt8772296',9),  ('tt4574334',10),
    ('tt0944947',10), ('tt0903747',10),  ('tt3032476',10), ('tt7366338',10),
    ('tt4786824',9),  ('tt10919420',8),  ('tt0141842',10), ('tt0773262',9),
    ('tt0108778',8),  ('tt1475582',10),  ('tt0475784',9)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
FROM (VALUES
    ('tt15239678',8), ('tt0468569',9), ('tt1517268',7), ('tt0816692',9),
    ('tt28015403',8), ('tt0944947',7), ('tt0108778',8), ('tt6751668',8)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
FROM (VALUES
    ('cinephile',     'tt15398776', 'Атомный реквием по гению',
     'Нолан отказался от привычной экшен-структуры и снял трёхчасовой биографический триллер, в котором главное оружие — диалог. Киллиан Мерфи играет Оппенгеймера как человека, проглотившего собственное изобретение и не способного его переварить. Чёрно-белые сцены допроса работают как ритм-секция: с каждой минутой петля затягивается всё туже. Это кино, которое заставляет понять, что такое моральный долг, не назидая ни секунды.', 10, 4820, 198),
    ('criticspeaks',  'tt15239678', 'Эпос пустыни без капли воды',
     'Вильнёв снял редкий случай продолжения, которое масштабнее и одновременно интимнее первой части. Песчаные планы здесь — не фон, а полноценный персонаж, давящий на героев. Сцена приручения червя — лучшая визуальная метафора последних лет о том, как покорный объект становится политической силой. Минус один балл за финальную трагическую линию Чани, которой явно не хватило экранного времени.', 9, 6210, 287),
    ('arthouse_fan',  'tt7160372', 'Звук как обвинение',
     'Глейзер сделал невозможное: снял фильм об Освенциме, в котором почти не показан Освенцим. Камера не покидает пределов комендантской дачи — а ужас просачивается через звуковую дорожку и мимоходом брошенные фразы. Это самый радикальный фильм о банальности зла со времён Ханны Арендт. Оставляет в полной тишине.', 10, 3140, 142),
    ('cinephile',     'tt14230458', 'Франкенштейн в кружевах',
     'Лантимос в очередной раз ставит эксперимент над зрителем — на этот раз с эстетикой викторианского стимпанка. Эмма Стоун проживает на экране все стадии взросления женщины за 140 минут и заслуженно получает Оскар. Камера с рыбьим глазом и пастельные тона создают мир, в котором нормальность перестаёт быть точкой отсчёта.', 9, 4520, 176),
    ('criticspeaks',  'tt28607951', 'Сказка с пинком под зад',
     'Шон Бэйкер снял сказку о Золушке, которая никогда не закончится. Майки Мэдисон взрывает экран — её Анора одновременно жёсткая, беззащитная, смешная и трагичная. Финальная сцена в машине стоит всех голливудских мелодрам последнего десятилетия. Каннская «Золотая пальмовая ветвь» здесь абсолютно по делу.', 9, 3890, 154),
    ('arthouse_fan',  'tt0079944', 'Зона как зеркало',
     'Фильм-молитва, фильм-загадка, фильм-проклятие. Тарковский оставил после себя визуальный текст, расшифровывать который будут ещё лет сто. Камера здесь движется с такой медлительностью, что это становится отдельной формой искусства. Хочется не смотреть, а вдыхать каждый кадр. Лучшее, что когда-либо появлялось в советской фантастике.', 10, 5210, 312),
    ('cinephile',     'tt0118767', 'Поколение, которое нас сделало',
     'Балабанов угадал нерв эпохи и снял фильм-портрет постсоветского человека, который не знает, что ему делать со свободой. Бодров играет Данилу с такой обескураживающей простотой, что начинаешь верить — этот парень действительно живёт по соседству. Музыка Бутусова стала гимном для двух поколений. Этот фильм просто нельзя обсуждать без эмоций.', 9, 7820, 423),
    ('weekendwatcher','tt0468569', 'Лучший комикс в истории',
     'Хит Леджер создал персонажа, который стал культурным феноменом до того, как фильм вышел в прокат. Нолан превратил блокбастер в криминальную драму — и это сработало. Сцена ограбления банка в начале до сих пор лучший пролог в супергеройском кино. Тёмный, умный, безжалостный.', 10, 9320, 521),
    ('marathonner',   'tt0944947', 'Сезонов восемь — претензий девять',
     'Первые шесть сезонов — это эпохальное телевидение, которое навсегда изменило индустрию. Финал, конечно, спорный, но прежде чем ругать — пересмотрите ту самую сцену в Тронном зале. Шоу с такой плотностью персонажей и интриг было до этого только у Шекспира. Минусую балл за драконицу, разбившую сердце фанатам.', 9, 11200, 612),
    ('series_lover',  'tt0903747', 'Превращение, после которого не вернуться',
     'Брайан Крэнстон сыграл лучшую роль на телевидении за всё время его существования. Винс Гиллиган выстроил пятисезонную драматургию с математической точностью — каждое ружьё стреляет, каждая деталь работает. Это шоу не о наркотиках, а о том, как человек теряет себя по миллиметру. Идеально.', 10, 14500, 789),
    ('cinephile',     'tt0068646', 'Образец жанра, эталон ремесла',
     'Коппола снял не фильм — мемориал американскому кинематографу. Каждый план — учебник по композиции, каждая сцена — мастер-класс по актёрской игре. Брандо в первые двадцать минут гипнотизирует одной интонацией. После «Крёстного отца» гангстерское кино перестало быть жанром и стало традицией.', 10, 8910, 456),
    ('criticspeaks',  'tt0111161', 'Надежда — хорошая штука',
     'Дарабонт превратил рассказ Кинга в гимн человеческому достоинству. Финал, разумеется, известен всем — но удивительно, как фильм продолжает работать даже на пятом просмотре. Морган Фриман и Тим Роббинс играют дружбу так, будто действительно знакомы двадцать лет. Это кино, которое не устаревает.', 10, 12300, 671),
    ('arthouse_fan',  'tt6751668', 'Этажи как метафора',
     'Пон Чжун Хо снял идеальный фильм о классовом неравенстве, не сказав ни одного дидактического слова. Архитектура здесь — главный сценарист: лестницы, подвалы, окна работают как социальный диагноз. Финальная двадцатиминутка по плотности драматургии не имеет аналогов в современном кино.', 10, 6800, 342),
    ('cinephile',     'tt0110912', 'Не сюжет — структура',
     'Тарантино перевернул нарратив с ног на голову и оказалось, что так смотреть кино только интереснее. Диалоги стали отдельной формой искусства. Сцена в кафе с разговором о бутербродах вошла во все учебники сценарного мастерства. Тридцать лет спустя — всё ещё свежо.', 10, 7100, 389),
    ('marathonner',   'tt7366338', 'Документ, который должен был быть снят',
     'Мейзин не сделал ни одной уступки зрителю. Сцена с горящим реактором отрезает все пути к комфорту — после неё досматривать уже невозможно остановиться. Джаред Харрис в роли Легасова — это тот случай, когда актёр становится памятником. Пять серий, которые меняют отношение к ответственности.', 10, 9800, 489),
    ('series_lover',  'tt2788316', 'Япония без клюквы',
     'FX и Disney+ собрали настоящих японских актёров, перевели сценарий обратно на японский и сняли историческую драму уровня «Семи самураев». Хироюки Санада в роли Торанаги — отдельный вид искусства. Сцены чайной церемонии напряжённее голливудских погонь. Лучший дебютный сезон со времён первого «Тру Детектива».', 10, 5400, 261),
    ('marathonner',   'tt28015403', 'Казань, которую мы забыли',
     'Крыжовников снял жестокую и честную хронику девяностых, не заигрывая с ностальгией. Подростки здесь — не герои и не жертвы, а продукты системы, которой больше не существует. Финал бьёт под дых. Лучший российский сериал последних пяти лет.', 9, 8200, 412),
    ('weekendwatcher','tt0167260', 'Финал, ради которого стоило ждать',
     'Питер Джексон закрыл трилогию так, что все претензии к двум первым частям моментально снимаются. Битва на Пеленнорских полях до сих пор остаётся эталоном масштабной баталии в кино. Возвращение Арагорна и финал в Серых Гаванях — момент, ради которого хочется пересматривать всё с самого начала.', 10, 6500, 298),
    ('weekendwatcher','tt0816692', 'Космос как метафора любви',
     'Нолан в очередной раз доказал, что блокбастер может быть умным. Часовая стрелка над Гаргантюа — лучшая визуализация теории относительности в художественном кино. Музыка Циммера превращает каждую сцену в литургию. Минус полбалла за чёрную дыру, которая ведёт к книжному шкафу — но это придирка.', 9, 8400, 421),
    ('series_lover',  'tt3032476', 'Спин-офф, превзошедший оригинал',
     'Винс Гиллиган и Питер Гулд сняли историю о том, как добрый человек медленно превращается в свою противоположность. Боб Оденкёрк играет Джимми МакГилла настолько живо, что воспринимаешь его как старого знакомого. Чёрно-белые финальные эпизоды — это уровень, на который большинство сериалов не поднимутся никогда.', 10, 5900, 287),
    ('cinephile',     'tt0047478', 'Эталон, переписавший правила',
     'Куросава придумал структуру, которую с тех пор повторяет всё мировое кино. От «Великолепной семёрки» до «Стражей Галактики» — все они дети «Семи самураев». 207 минут, в которых нет ни секунды лишнего хронометража. Кино, после которого хочется выучить японский.', 10, 4100, 218),
    ('criticspeaks',  'tt2802154', 'Современное прочтение Книги Иова',
     'Звягинцев снял библейский сюжет в декорациях постсоветской провинции и попал в нерв страны. Алексей Серебряков играет своего Николая как человека, у которого государство отнимает не дом, а смысл. Жесткое, неудобное, обязательное к просмотру кино.', 9, 4700, 234),
    ('series_lover',  'tt0773262', 'Серийный убийца как обаятельный сосед',
     'Майкл С. Холл сыграл такого Декстера, которому начинаешь сочувствовать против собственной воли. Сценаристы первых четырёх сезонов выстроили моральную игру с зрителем на грани этики. Финал — отдельная боль, но первые сезоны искупают всё.', 9, 4300, 187),
    ('weekendwatcher','tt1517268', 'Розовое стекло, через которое всё видно',
     'Грета Гервиг сняла фильм по игрушке так, будто это был серьёзный ирландский артхаус. Маргот Робби играет Барби с такой самоиронией, что не верится, что это блокбастер. Монолог Глории о женских противоречиях — лучший момент массового кино за год. Не идеально, но смело.', 8, 11200, 567),
    ('arthouse_fan',  'tt13238346', 'Тихая мелодрама о невозможности',
     'Селин Сонг сняла дебютный фильм с тактом и зрелостью, которые приходят к режиссёрам через десятилетия. Грета Ли играет женщину, которая выбрала жизнь, и не жалеет об этом — но и не уверена. Финал на пирсе — три минуты, после которых неделю не можешь говорить.', 10, 3700, 178),
    ('cinephile',     'tt0033467', 'Кино, которое изобрело язык',
     'Уэллс в 25 лет переписал учебник кинематографа. Глубинная мизансцена, низкие точки съёмки, флешбэки внутри флешбэков — всё, что мы считаем нормой, придумано здесь. «Розочка» как символ невозвратной невинности — гениально и до сих пор не превзойдено.', 9, 2900, 145),
    ('criticspeaks',  'tt1475582', 'Холмс, который стал нашим современником',
     'Бенедикт Камбербэтч и Мартин Фримен создали химию, на которой держится весь сериал. Стивен Моффат и Марк Гэтисс смогли пересказать классические сюжеты Дойля так, что они зазвучали свежо. Минусую за провисший четвёртый сезон — но первые три безусловно входят в золотой фонд.', 9, 6800, 312),
    ('marathonner',   'tt0141842', 'Шоу, после которого появилась эпоха',
     'Дэвид Чейз создал прототип всего современного телевидения. Тони Сопрано в исполнении Джеймса Гандольфини — гангстер у психотерапевта, который кормит уток в бассейне и плачет. Семья как метафора, метафора как способ говорить о смерти американской мечты. Эталон.', 10, 7300, 389),
    ('arthouse_fan',  'tt14549466', 'Кроненберг возвращается к телу',
     'Канадский мастер боди-хоррора не теряет хватки. Вигго Мортенсен играет художника, который выращивает в себе новые органы — и это метафора нашего времени с пугающей точностью. Не для всех, но для тех, кто примет правила игры — невероятный опыт.', 8, 1800, 87),
    ('weekendwatcher','tt0231507', 'Продолжение, которое стало эпохой',
     'Балабанов снял свой первый блокбастер и попал в больной нерв сразу нескольких поколений. Песня «В Питере — пить» здесь и появилась бы наверняка, если бы Шнур её ещё не написал. Сцена на мосту в Чикаго — лучший момент русского кино двухтысячных.', 9, 9100, 478)
...
SELECT (SELECT id FROM users WHERE username='cinephile'), c.id, v.val
FROM (VALUES
    ('tt5040012',9), ('tt17526714',8), ('tt20215234',9), ('tt0056801',10),
    ('tt0017136',10), ('tt0114369',9), ('tt0102926',10), ('tt0083658',10),
    ('tt0062622',10), ('tt0078748',9), ('tt0050976',10), ('tt1832382',9),
    ('tt0245429',10), ('tt23203364',8), ('tt12990770',9)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
SELECT (SELECT id FROM users WHERE username='criticspeaks'), c.id, v.val
FROM (VALUES
    ('tt5040012',7), ('tt17526714',8), ('tt20215234',8), ('tt12037194',8),
    ('tt1865505',5), ('tt0056801',9), ('tt0114369',9), ('tt0102926',10),
    ('tt0133093',8), ('tt2356777',9), ('tt0306414',10), ('tt7660850',10),
    ('tt5687612',9), ('tt0386676',8), ('tt23203364',7)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
SELECT (SELECT id FROM users WHERE username='marathonner'), c.id, v.val
FROM (VALUES
    ('tt3581920',10), ('tt9253284',9), ('tt11280740',10), ('tt5678012',8),
    ('tt11041332',8), ('tt2356777',10), ('tt2802850',9), ('tt12990770',9),
    ('tt0411008',9), ('tt0306414',10), ('tt7660850',10), ('tt0386676',9),
    ('tt8111088',8), ('tt14452776',9)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
SELECT (SELECT id FROM users WHERE username='weekendwatcher'), c.id, v.val
FROM (VALUES
    ('tt12037194',8), ('tt0103064',10), ('tt0133093',10), ('tt0088763',10),
    ('tt0078748',8), ('tt0114369',8), ('tt0110357',9), ('tt0114709',9),
    ('tt8111088',9), ('tt1190634',8), ('tt3581920',9), ('tt0386676',10),
    ('tt23203364',8)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
SELECT (SELECT id FROM users WHERE username='arthouse_fan'), c.id, v.val
FROM (VALUES
    ('tt17526714',9), ('tt0056801',10), ('tt0017136',10), ('tt0050976',10),
    ('tt1832382',10), ('tt0369702',9), ('tt27713378',9), ('tt12990770',10),
    ('tt0098936',10), ('tt5687612',9), ('tt0306414',10),
    -- мейнстрим — низко
    ('tt12037194',5), ('tt0133093',6), ('tt1190634',5)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
SELECT (SELECT id FROM users WHERE username='series_lover'), c.id, v.val
FROM (VALUES
    ('tt3581920',10), ('tt9253284',10), ('tt11280740',10), ('tt5678012',9),
    ('tt11041332',8), ('tt2356777',10), ('tt2802850',9), ('tt12990770',9),
    ('tt0411008',9), ('tt0306414',10), ('tt0804503',10), ('tt0098936',9),
    ('tt7660850',10), ('tt14452776',9), ('tt5290382',9)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
SELECT (SELECT id FROM users WHERE username='casual_viewer'), c.id, v.val
FROM (VALUES
    ('tt5040012',7), ('tt12037194',8), ('tt0088763',9), ('tt0103064',9),
    ('tt0133093',8), ('tt0110357',9), ('tt8111088',8), ('tt3581920',8),
    ('tt0386676',9), ('tt23203364',7)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
FROM (VALUES
    ('tt9253866',10), ('tt5491994',10), ('tt0306414',10), ('tt5290382',9),
    ('tt0102926',8), ('tt0114369',8), ('tt0050976',9), ('tt1832382',10),
    ('tt0369702',9), ('tt23203364',7), ('tt30127432',8)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
FROM (VALUES
    ('tt23203364',9), ('tt30127432',8), ('tt27915934',7), ('tt5151318',9),
    ('tt13561804',8),
    -- классика и существующее
    ('tt0118767',10), ('tt0231507',10), ('tt0079944',10), ('tt2802154',9),
    ('tt0066565',9), ('tt28015403',10)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
FROM (VALUES
    ('tt0088763',10), ('tt0103064',10), ('tt0133093',10), ('tt0114369',9),
    ('tt0102926',10), ('tt0110357',10), ('tt0114709',9), ('tt0119698',9),
    ('tt0096697',9), ('tt0098936',10), ('tt0306414',9),
    ('tt0118767',10)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
FROM (VALUES
    ('tt5040012',9), ('tt17526714',9), ('tt12235712',7), ('tt0078748',10),
    ('tt0102926',9), ('tt0114369',9), ('tt11041332',8), ('tt27915934',8),
    ('tt5290382',8), ('tt3581920',9), ('tt0098936',10), ('tt0096697',7)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
FROM (VALUES
    ('tt0386676',10), ('tt5687612',10), ('tt4288182',9), ('tt7660850',9),
    ('tt2802850',9), ('tt14452776',9), ('tt0096697',9), ('tt0114709',8),
    ('tt2948356',9), ('tt0088763',9), ('tt13561804',8), ('tt1190634',8)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
FROM (VALUES
    ('tt0017136',10), ('tt0050976',10), ('tt0056801',10), ('tt0062622',10),
    ('tt0083658',9), ('tt0078748',9), ('tt0102926',9),
    -- из существующего: классика
    ('tt0033467',10), ('tt0047478',10), ('tt0079944',10), ('tt0036824',9)
) AS v(imdb,val)
JOIN content c ON c.imdb_id = v.imdb
ON CONFLICT (user_id, content_id) DO NOTHING;
...
FROM (VALUES
    ...
    ('criticspeaks', 'tt20215234', 'Шепот в Сикстинской капелле',
     'Бергер снял триллер, в котором главное оружие — пауза. Файнс держит фильм на плечах одной интонацией. Сцены тайного голосования напряжённее любого экшена. Финальный твист спорный, но до него вы уже окончательно попадёте под чары этой холодной красоты. Пример того, как умное кино может быть зрелищным.', 9, 5100, 218),
    ('weekendwatcher', 'tt12037194', 'Возвращение к корням саги',
     'Миллер доказал, что в 79 лет можно снимать самые безумные погони года. Аня Тейлор-Джой тащит фильм одним взглядом — диалогов у неё минимум. Сцена с парапланом — лучший экшен, что я видел за последние пять лет. Минусую за провисший пролог, но финал искупает всё.', 8, 6700, 312),
    ('cinephile', 'tt0056801', 'Автопортрет в кризисе',
     'Феллини в этом фильме сделал то, что до него не делал никто: показал режиссёра, который не знает, что снимать, и превратил это в кино. Марчелло Мастроянни играет себя и Феллини одновременно. Финальный круговой танец — самый честный финал в истории кинематографа. Эталон самокритики.', 10, 2800, 134),
    ('silent_era', 'tt0017136', 'Будущее, изобретённое сто лет назад',
     'Ланг придумал визуальный язык, по которому до сих пор живёт всё научно-фантастическое кино. Городская архитектура, машинные интерьеры, образ робота-Марии — всё это родилось здесь. То, что фильм 1927 года смотрится свежо в 2025-м, говорит само за себя. Восстановленная версия с найденными сценами обязательна к просмотру.', 10, 1900, 98),
    ('cinephile', 'tt0114369', 'Финчер задаёт стандарт',
     'Финчер взял жанровый триллер и превратил его в средневековый моралитет. Каждый кадр снят так, будто Калигари держал камеру. Морган Фриман и Брэд Питт играют одну из лучших напарнических пар в истории жанра. Финал в коробке — момент, после которого жанр уже не тот.', 10, 5400, 234),
    ('cinephile', 'tt0102926', 'Гипноз в железной клетке',
     'Энтони Хопкинс заработал Оскар за 16 минут экранного времени и заслужил каждую секунду. Джонатан Демми снял триллер, который держится не на саспенсе, а на психологии каждой сцены. Джоди Фостер играет уязвимость и стальную волю одновременно. Безупречный фильм во всех смыслах.', 10, 6100, 287),
    ('cinephile', 'tt0083658', 'Спор о том, что значит быть человеком',
     'Ридли Скотт собрал вместе нуар, киберпанк и философский трактат — и получился шедевр. Финальный монолог Рутгера Хауэра «Tears in rain» — три минуты, ради которых стоит пересматривать весь фильм. Режиссёрская версия лучше театральной по всем параметрам. Кино, которое создало целый жанр.', 10, 4700, 201),
    ('cinephile', 'tt0062622', 'Молчание как сюжет',
     'Кубрик не снимает кино — он ведёт зрителя через медитацию о месте человека во Вселенной. Первые двадцать минут без слов — самый радикальный пролог в истории. Сцена с HAL 9000, поющим Daisy, ломает все каноны эмпатии. Если у вас нет терпения — пропустите. Если есть — это лучшее, что можно увидеть.', 10, 3900, 178),
    ('horror_lover', 'tt0078748', 'Бог хоррора',
     'Ридли Скотт собрал на маленькой замкнутой локации идеальный фильм ужасов. Чужой как существо до сих пор не имеет аналогов в дизайне монстров. Сигурни Уивер создала образ женщины-героя, переделавший правила жанра. Сцена с грудью на ужине — момент, который останется в памяти навсегда.', 10, 5800, 245),
    ('weekendwatcher', 'tt0088763', 'Идеальный летний блокбастер',
     'Земекис снял фильм, в котором всё работает: химия Майкла Дж. Фокса и Кристофера Ллойда, монтаж, музыка Алана Сильвестри, шутки на всех уровнях. Финал, в котором всё связывается одним кадром — учебник сценарного ремесла. Сорок лет спустя — никаких морщин.', 10, 7200, 318),
    ('weekendwatcher', 'tt0103064', 'Сиквел, переплюнувший оригинал',
     'Кэмерон взял низкобюджетный хоррор и превратил его в эпический боевик с философским подтекстом. Шварценеггер играет на инверсии — герой, защищающий мать, которую он раньше пришёл убить. T-1000 стал визитной карточкой жидкого металла на десятилетия вперёд. Эталон сиквела.', 10, 6500, 287),
    ('cinephile', 'tt0245429', 'Магия Миядзаки в чистом виде',
     'Миядзаки сделал то, что не удавалось до него никому: создал анимационную сказку, понятную детям и взрослым с равной глубиной. Каждый кадр — самостоятельная картина. Без-Лица как метафора потребления, Хаку как мост между мирами — всё работает на нескольких уровнях. Заслуженный Оскар.', 10, 4100, 198),
    ('cinephile', 'tt0119698', 'Без однозначных героев',
     'Миядзаки разрушает шаблон сказки: здесь нет злодеев, только герои с разными правдами. Госпожа Эбоси не зло — она строит новый мир. Принцесса Мононоке защищает старый. Аситака — мост, который пытается их примирить. Финальная сцена за Шиши-гами — кульминация всей экологической мысли мастера.', 10, 3700, 176),
    ('weekendwatcher', 'tt0110357', 'Король анимации',
     'Disney взял Гамлета и переписал его для детей, не потеряв трагической глубины. Сцена смерти Муфасы остаётся одной из самых эмоциональных в истории мультипликации. Музыка Элтона Джона и Ханса Циммера — отдельный шедевр. Не зря этот фильм окупился в десять раз.', 9, 8100, 421),
    ('comedy_addict', 'tt0114709', 'Pixar изобретает будущее',
     'Когда Pixar выпустил «Историю игрушек», никто не ожидал, что компьютерная анимация может рассказывать такие истории. Дружба Вуди и Базза — учебник по характерам в детском кино. Каждая шутка работает на разном уровне. Старт революции анимационного кино.', 9, 5200, 234),
    ('series_lover', 'tt3581920', 'Видеоигра, ставшая шедевром ТВ',
     'Крейг Мейзин и Нил Дракманн совершили невозможное: сняли экранизацию игры, которая работает как самостоятельная драма. Третий эпизод первого сезона про Билла и Фрэнка — лучшая короткометражная любовная история, что я видел за десять лет. Педро Паскаль и Белла Рамзи играют одну из лучших пар в современной телепродукции.', 10, 9800, 487),
    ('series_lover', 'tt9253284', 'Звёздные войны для взрослых',
     'Тони Гилрой освободил вселенную от джедайской мифологии и снял шпионский триллер о революции. Речь Карис Не Дханни в финале — одна из лучших политических речей на современном телевидении. Стелан Скарсгард в роли Луфена Раэля — отдельный вид искусства. Лучший продукт во вселенной за два десятилетия.', 10, 6700, 332),
    ('series_lover', 'tt11280740', 'Шедевр корпоративной жути',
     'Бен Стиллер снял сериал, в котором каждый кадр работает на тревогу. Концепция «разделённых» сотрудников — лучшая метафора современного офисного отчуждения. Адам Скотт играет одну из самых тонких ролей в своей карьере. Финал второго сезона — час напряжения, после которого хочется сразу пересмотреть весь сезон.', 10, 8400, 412),
    ('marathonner', 'tt5678012', 'Шпионы среди мусорных пакетов',
     'Гэри Олдман в роли Джексона Лэмба — это не роль, это явление. Сериал работает на инверсии: главные герои здесь — неудачники из MI5, которым доверяют только то, чем брезгуют другие. Уилл Смит, написавший шесть романов про Slough House — это новый Ле Карре. Сериал на годы вперёд.', 9, 4200, 198),
    ('arthouse_fan', 'tt2356777', 'Атмосфера как герой',
     'Первый сезон с Макконахи и Харрельсоном переписал правила телевизионного крайма. Восьмиминутный непрерывный план в четвёртой серии — техническое чудо. Философские монологи Раста Коула — материал, по которому можно писать диссертации. Остальные сезоны слабее, но первый — эталон.', 10, 7100, 354),
    ('marathonner', 'tt2802850', 'Снег и абсурд',
     'Ноа Хоули переосмыслил вселенную братьев Коэн в долгоиграющем формате — и каждый сезон работает как отдельный фильм. Билли Боб Торнтон в первом сезоне, Кирстен Данст во втором, Юэн МакГрегор в третьем — все на пике формы. Чёрный юмор и моральная серьёзность в идеальной пропорции.', 9, 5800, 276),
    ('arthouse_fan', 'tt12990770', 'Восемь часов чёрно-белого гипноза',
     'Стивен Заиллян снял мини-сериал, который смотрится как продлённый итальянский нуар. Эндрю Скотт играет Тома Рипли с ледяной точностью социопата. Роберт Элсвит — оператор, который превращает каждый кадр в музейный экспонат. Мелодраматизм Хайсмит здесь сменён холодным наблюдением — и это работает.', 9, 3500, 167),
    ('documentaries_only', 'tt9253866', 'Образцовый научпоп',
     'Дэвид Аттенборо снова делает то, что умеет лучше всех: рассказывает о природе так, что хочется бросить всё и записаться в волонтёры WWF. Команда оператора Алистера Фотергилла снимала проект четыре года в пятидесяти странах. Кадры с китами и пингвинами — эталон жанра. Обязательный просмотр.', 10, 4800, 234),
    ('horror_lover', 'tt11041332', 'Двойная временная линия',
     'Юрий Брэйс и Эшли Лайл создали редкий случай шоу, в котором обе временные линии одинаково увлекательны. Кристина Риччи и Джульетта Льюис — выдающиеся актрисы, которые наконец получили роли по своему калибру. Третий сезон чуть слабее, но первые два — must-watch.', 8, 3700, 156),
    ('comedy_addict', 'tt5687612', 'Нарушение четвёртой стены как искусство',
     'Фиби Уоллер-Бридж за 12 эпизодов сделала больше для женской комедии, чем многие за десятилетия. Прямые взгляды в камеру здесь работают не как трюк, а как способ быть честной с зрителем. Эпизод со священником во втором сезоне — лучшая сцена в комедийном телевидении 2010-х. Точно, остро, безжалостно к себе.', 10, 5400, 278),
    ('comedy_addict', 'tt0386676', 'Ситком, который определил эпоху',
     'Грег Дэниелс адаптировал британский «Офис» так, что американская версия превзошла оригинал. Стив Карелл создал образ Майкла Скотта, который не имеет аналогов в мире комедии — одновременно нелепый и трогательный. Девять сезонов, ни один из которых не хочется пропустить. Эталон.', 10, 9300, 478),
    ('arthouse_fan', 'tt0306414', 'Социология в форме драмы',
     'Дэвид Саймон и Эд Бёрнс создали не сериал, а социальный документ о современном американском городе. Каждый из пяти сезонов фокусируется на новой институции — наркоторговле, полиции, школе, прессе, политике. Без Майкла К. Уильямса в роли Омара мир был бы беднее. Абсолютный эталон телевидения.', 10, 5600, 289),
    ('russian_cinema_fan', 'tt23203364', 'Локшин нашёл ключ к Булгакову',
     'После провалов всех предыдущих экранизаций Локшин предложил радикальный ход: не пересказывать роман, а ставить кино о том, как пишется роман. Аугуст Диль в роли Воланда — одна из лучших актёрских работ в российском кино. Юлия Снигирь — настоящая Маргарита. Минусую за слабую вторую половину, но первые полтора часа — событие.', 9, 7800, 354)
) AS v(uname,imdb,title,body,rv,vc,lc)
JOIN users u ON u.username = v.uname
JOIN content c ON c.imdb_id = v.imdb
WHERE NOT EXISTS (
